/**
 *  @file testfloat_CompDecomp_libpressio.c
 *  @author Sheng Di
 *  @date April, 2015
...
int main(int argc, char * argv[])
{
    ...
    if(argc < 9)
    {
	printf("Test case: testfloat_CompDecomp_libpressio [config_file] [zc.config] [solName] [varName] [errBoundMode] [err bound] [srcFilePath] [dimension sizes...]\n");
	printf("Example: testfloat_CompDecomp_libpressio sz.config zc.config sz(1E-6) testfloat ABS 1E-6 testdata/x86/testfloat_8_8_128.dat 8 8 128\n");
	exit(0);
    }
...
//    float *data = readFloatData(oriFilePath, &nbEle, &status);
    ...
    struct pressio_data* dims = pressio_data_new_empty(pressio_float_dtype, dim, rdim);
    ...
    float* data = pressio_data_ptr(input_data, NULL);
    ...
    float* decData = pressio_data_ptr(decompressed_data, NULL);
