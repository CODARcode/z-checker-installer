...
int main(int argc, char *argv[])
{
  ...
  struct pressio_compressor* compressor = pressio_get_compressor(library, "fpzip");
  struct pressio_options* fpzip_options = pressio_compressor_get_options(compressor);
  ...
  if(argc < 7)
  {
        printf("Test case: fpzip_CompDecomp_float [config_file] [solName] [varName] [precision] [srcFilePath] [dimension sizes...]\n");
        printf("Example: fpzip_CompDecomp_float zc.config sol var 28 testdata/x86/testfloat_8_8_128.dat 8 8 128\n");
        exit(0);
  }
  ...
  // configure the compressor
   ...
   pressio_options_set_integer(fpzip_options, "fpzip:prec", prec);
   
   if(pressio_compressor_check_options(compressor, fpzip_options)) {
    printf("%s\n", pressio_compressor_error_msg(compressor));
    exit(pressio_compressor_error_code(compressor));
  }
  if(pressio_compressor_set_options(compressor, fpzip_options)) {
    printf("%s\n", pressio_compressor_error_msg(compressor));
    exit(pressio_compressor_error_code(compressor));
  }
