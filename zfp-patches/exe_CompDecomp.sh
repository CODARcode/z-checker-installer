...
if [[ $option == 1 ]]; then
	...
	do
		...
		echo ./zfp_CompDecomp.sh zc.config "${compressor}($absErrBound)" "$datatype" "$file" $errBoundMode $absErrBound "$dataDir/$file" $dim1 $dim2 $dim3 $dim4
		./zfp_CompDecomp.sh zc.config "${compressor}($absErrBound)" "$datatype" "$file" $errBoundMode $absErrBound "$dataDir/$file" $dim1 $dim2 $dim3 $dim4
	done
else
	...
	for (( i = 0; i < nbVars; i++)); do
		...
		echo ./zfp_CompDecomp.sh zc.config "${compressor}($absErrBound)" "$datatype" "$varName" $errBoundMode $absErrBound "$dataDir/$file" $dims
		./zfp_CompDecomp.sh zc.config "${compressor}($absErrBound)" "$datatype" "$varName" $errBoundMode $absErrBound "$dataDir/$file" $dims
	done
fi
