...
void usage()
{
	...
	printf("	-j <json config. file>: configuration file\n");
...
char* read_file(const char* filepath) {
  ...
  char * json_str = malloc(json_stat.st_size);
  if(json_str == NULL) {
...
int loadJsonFile(char* zc_cfgFile, char** compressorName, char** compressorMode, char** compressor, char** workspaceDir, char** exeDir, char** preCommand, char** exeCommand)
{
	...
	char* json_str = read_file(zc_cfgFile);
	...
	struct pressio_options_iter* iter = pressio_options_get_iter(json_options);
	...
	while(pressio_options_iter_has_value(iter))
	{
		...
		if(strcmp(key, "zchecker:compressor_name")==0)
			strcpy(*compressorName, pressio_option_get_string(option));
		if(strcmp(key, "zchecker:compressor_mode")==0)
			strcpy(*compressorMode, pressio_option_get_string(option));
		if(strcmp(key, "zchecker:full_compressor_name")==0)
			strcpy(*compressor, pressio_option_get_string(option));
		if(strcmp(key, "zchecker:workspaceDir")==0)
			strcpy(*workspaceDir, pressio_option_get_string(option));
		if(strcmp(key, "zchecker:exeDir")==0)
			strcpy(*exeDir, pressio_option_get_string(option));
		if(strcmp(key, "zchecker:preCommand")==0)
			strcpy(*preCommand, pressio_option_get_string(option));
		if(strcmp(key, "zchecker:exeCommand")==0)
			strcpy(*exeCommand, pressio_option_get_string(option));
		pressio_option_free(option);
		pressio_options_iter_next(iter);	
	}

	printf("compressorName=%s\n", *compressorName);
	printf("compressorMode=%s\n", *compressorMode);
	printf("compressor=%s\n", *compressor);
...
int processCreateZCCase(int operation, char* compressorName, char* mode, char* compressor, char* workspaceDir, char* exeDir, char* preCommand, char* exeCommand, char* confFilePath)
{
	...
	else if(operation == ADD_CMPR)
	{
		...
		char  workspaceDir_fullPath[PATH_MAX+1];
 		...
 		if(!res)
		{
			printf("cannot find the path of the file: %s \n", res);
			return 1;
		}
...
int main(int argc, char* argv[])
{
	...
	{
		int loadStatus = loadJsonFile(jsonFile, &compressorName_, &mode_, &compressor_, &workspaceDir_, &exeDir, &preCommand, &exeCommand);
		if(loadStatus!=0)
		{
			printf("Error: wrong configuration file\n");
			exit(0);
